...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.98\columnwidth]{figures/fig5_scene_schematic.pdf}
    \caption{\textbf{Genesis edge-push benchmark across material families.} Each panel is a rendered frame from the Genesis simulator after the pile has settled on the source platform. Scene geometry (Franka Panda, source platform, target bin, particle pile placement following Config~D, 5.5\,cm from the open edge) is identical across panels; only the MPM material parameters differ, producing visibly different equilibrium shapes---granular sand spreads, cohesive snow holds together loosely, and elastoplastic media retain a tight rebounded block. Particle surface colours are set per panel for visual contrast and have no effect on dynamics.}
