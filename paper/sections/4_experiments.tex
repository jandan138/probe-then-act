...
\begin{figure}[t]
    ...
    \caption{\textbf{Edge-push benchmark scene.} Top: top-down view of the same scene instantiated with each material family (sand / snow / elastoplastic); the particle pile sits on a small elevated source platform and the policy must push it across the red edge into the target bin without spilling. Bottom: side view (X--Z) showing the source pedestal, particle pile, scoop end-effector, and edge marker. The scene geometry (platform, bin, pile size and position) is identical across material families; only the MPM material parameters differ.}
    \label{fig:scene_schematic}
\end{figure}

\cref{tab:benchmark} summarizes the five material splits. \cref{fig:scene_schematic} shows the corresponding scene geometry; the same Franka, scoop, source platform, target bin, and pile placement (Config D, 5.5\,cm from the open edge) are used for every split, so cross-split differences come from material parameters alone. We use Genesis MPM at grid density 128 with 25 substeps per control step (control $\Delta t = 2 \times 10^{-3}$~s).
