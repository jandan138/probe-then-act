...
\subsection{The Recoverable-Deformation Hypothesis as a Candidate Heuristic}

The hypothesis is not validated by the present benchmark: the positive evidence comes from a single elastoplastic family, and recoverability is not isolated from confounders (task ease, particle density, control stability, sand-only training, the simulator's specific elastoplastic constitutive model). We offer it as a physically grounded interpretation of the asymmetry and as a design rule to be tested in future work.
...
\begin{itemize}
    ...
    \item \textbf{Cohesive granular media} (wet sand, snow): partly recoverable in principle. Snow showed a 13.3pp loss in our setup; the broader regime is not tested.
\end{itemize}

This is a scope claim, not a universal recipe. A robotic stack handling mixed materials should include a coarse material-class router that selects between \PTA{} and a reactive policy.

\subsection{Why the Privileged Teacher Fails on the Elastoplastic Split}
