...
We formalize \PTA{} as a two-stage decision process that augments a standard finite-horizon MDP with an explicit probing phase and a latent task-conditioning step. The formulation makes explicit that probing changes both the information available to the network and the physical state distribution faced by the task policy.

\subsection{Problem Formulation}

An episode consists of a robot interacting with a material instance $m \in \calM$ drawn from a distribution over hidden physical properties (Young's modulus $E$, Poisson ratio $\nu$, density $\rho$, cohesion, and plasticity parameters). The robot receives observation $s_t \in \R^{d_s}$ at step $t$ and applies action $a_t \in \R^{d_a}$. The observation contains proprioceptive state (joint positions, end-effector pose, time fraction) and aggregate particle statistics computed from simulator state---mean $y$-coordinate, transfer fraction into the target region, and spill fraction off the platform---but \emph{not} the ground-truth material parameters $m$. The task reward combines transfer, spill, and success terms defined for our edge-push benchmark; Genesis provides the simulator.

Our goal is to learn a policy $\pi_\theta(a_t \mid s_t, z, \sigma)$ that maximizes expected episode return over the material distribution, where $(z, \sigma)$ is a latent task-conditioning vector produced by a paired encoder $q_\phi$ from a short probing rollout. In the claim-bearing matched protocol, $q_\phi$ is a fixed randomly initialized sidecar saved and evaluated with the policy checkpoint; the policy is trained to use that matched latent coordinate system. We refer to $(z, \sigma)$ as a ``belief'' for brevity but do not interpret it as a calibrated posterior: $\sigma$ is an encoder scale output, not a posterior standard deviation. $m$ is never observed; $z$ is derived entirely from the probing interaction, and its usefulness depends on the post-probe state as well as the encoded material response.
...
\paragraph{Encoder.}
A two-layer MLP with hidden size 256 maps each probe step independently to a feature vector, mean-pools across steps, and emits $(\mu_z, \log \sigma_z^2) \in \R^{16} \times \R^{16}$ via a linear head. We sample $z \sim \Normal(\mu_z, \diag(\sigma_z^2))$ at training and use $z = \mu_z$ at evaluation. The matched protocol saves this encoder sidecar with the policy checkpoint and reloads the same sidecar for evaluation; no material-label objective, KL regularizer, or calibration loss is used.
...
The probe phase runs first; the paired encoder produces $(\mu_z, \sigma_z)$ from $\tau_{\mathrm{probe}}$; the task phase is rolled out with $\pi_\theta(\cdot \mid s_t, \mu_z, \sigma_z)$. PPO trains the policy parameters $\theta$ while keeping the probe trajectory fixed; in the matched sidecar protocol, evaluation reloads the same encoder paired with that policy rather than constructing a fresh encoder. Advantages and returns are computed over the task phase only, so the policy is shaped by the downstream usefulness of the matched latent coordinates. Because the probe trajectory is fixed open-loop, gradients do not flow into the probing actions; the policy learns how to use the pre-specified probing response rather than to optimize the probe.
