...
def main() -> None:
    ...
    scene.build()
    print("MINIMAL_MAINTHREAD_VIEWER_BUILT", flush=True)
    ...
    def step_loop() -> None:
        try:
            ...
            print(f"STEP_LOOP_EXCEPTION: {exc}", flush=True)
            stop.set()

    worker = threading.Thread(target=step_loop, daemon=True)
    worker.start()
    ...
    try:
        ...
        print("MINIMAL_MAINTHREAD_VIEWER_EXITED", flush=True)
